...
<head>
    ...
    <title>Update Medical Machine Request</title>
            ...
            <div class="main-content">
                <section class="section">
                    ...
                    <div class="col-10 col-md-6 col-lg-12">
                        <div class="card">
                            <form method="POST" action="{{ route('doctor.medicalMachineRequest.update' , $medicalMachineRequest->id)}}"
                                enctype="multipart/form-data">
                                @csrf
                                @method('put')
                                <div class="card-header">
                                    <h4>Update Medical Machine Request</h4>
                                </div>
                                                    ...
                                                    <select class="form-control" name="medicalMachineID"
                                                        ...
                                                        @foreach ($medicalMachines as $medicalMachine)
                                                            <option value="{{ $medicalMachine->id }}">
                                                                {{ $medicalMachine->name }} {{$medicalMachine->id == $medicalMachineRequest->medicalMachineID ? 'selected' : '' }} </option>
                                                        @endforeach
                                                    </select>
                                            </div>
